...
```
*Extract the `accessToken` and `company.id` from response.*
...
```
*Extract the job `id` and `assessmentUrl`.*
...
```
*Response: 400 VALIDATION_ERROR (Skill level below minimum).*
...
```
*Response: 200 OK (Status updated to SHORTLISTED).*
